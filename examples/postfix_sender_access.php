  ...
  switch ($submit) {
   ...
   case "Edit":
	...
	if ($bulk) {
        ...
        echo "&nbsp<a href=\"".$sess->self_url()."\">Back to postfix_sender_access.</a><br>\n";
        page_close();
        exit;
     }
    } else {
...
switch ($cmd) {
    ...
    default:
	$cmd="Query";
	$t = new postfix_sender_accessTable;
	$t->heading = 'on';
	$t->add_extra = 'on';
	$t->add_insert = 'my_postfix_sender_accessform';
	$t->edit = 'my_postfix_sender_accessform';
	$db = new DB_postfix;

        //echo "&nbsp<a href=\"".$sess->self_url()
	//	.$sess->add_query(array("cmd"=>"Add"))."\">Add postfix _sender _access</a>&nbsp\n";
  ...
  // there is no .
  if (!isset($q)) {
    $q = new postfix_sender_access_Sql_Query;     // We make one
  ...
  if ($query) {
    ...
    // Do that query
    $db->query("select * from postfix_sender_access where ". $query);
