...
class my_postfix_client_accessform extends postfix_client_accessform {
	var $classname="my_postfix_client_accessform";
}

$f = new my_postfix_client_accessform;
  ...
  switch ($submit) {
   ...
   case "Edit":
    if (isset($auth)) {
     if (!$f->validate()) {
        $cmd = $submit;
        echo "<font class=bigTextBold>$cmd postfix _client _access</font>\n";
        $f->display();
        page_close();
        exit;
     }
	...
	$modified = date("Y-m-d H:m:s");
        ...
        echo "&nbsp<a href=\"".$sess->self_url()."\">Back to postfix_client_access.</a><br>\n";
        page_close();
        exit;
     }
    } else {
...
switch ($cmd) {
    ...
    default:
	$cmd="Query";
	$t = new postfix_client_accessTable;
	$t->heading = 'on';
	$t->add_extra = 'on';
	$t->add_insert = 'my_postfix_client_accessform';
	$t->edit = 'my_postfix_client_accessform';
	$db = new DB_postfix;

  //      echo "&nbsp<a href=\"".$sess->self_url()
//		.$sess->add_query(array("cmd"=>"Add"))."\">Add postfix _client _access</a>&nbsp\n";
  //      echo "&nbsp<a href=\"".$sess->url("/index.php")."\">Home</a>&nbsp\n";
	echo "<br><font class=bigTextBold>Internal IP Ranges</font>\n";
  ...
  // there is no .
  if (!isset($q)) {
    $q = new postfix_client_access_Sql_Query;     // We make one
    ...
    $q->sortorder  = "inet_aton(start_block),reverse(start_block)";
  ...
  if (empty($query)) { $query="id!='0'"; }
  $db->query("SELECT COUNT(*) as total from postfix_client_access where ".$query);
  $db->next_record();
  ...
  $sortorder = str_replace("start_block","INET_ATON(start_block),start_block",$sortorder);
  if (!$sortorder) $sortorder="inet_aton(start_block),reverse(start_block)";
  if (!strpos("order by",$query)) $query .= " order by $sortorder";

  $query .= " LIMIT ".$q->start_row.",".$q->row_count;


  $query = "id!='0' order by inet_aton(start_block)";
...
//    printf("Query Condition = %s<br>\n", $query);
    ...
    // Do that query
    $db->query("select * from postfix_client_access where ". $query);
