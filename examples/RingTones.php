   ...
   case "Send Message":
	...
	if (($Credits>$MsgCount) || (!$perm->have_perm("guest"))) {
		...
		if ($db->next_record()) {
            ...
            sleep(3); $RetryCount=0;
            $endBalance = $mysms->getbalance();
            while ($endBalance==$startBalance and $RetryCount<5) {
                   sleep(2);
                   $RetryCount++;
                   $endBalance = $mysms->getbalance();
            }
...
if ($perm->have_perm("admin"))
  ...
  default:
    ...
    echo "<h3>SMS Sender - Nokia - Ring Tone Text Transfer Language</h3>";
    echo "<h4><a href=nokring_full.zip>Download NokRing RTTTL Composer</a> ";
    echo "to Listen to or compose your Ring Tones before you SMS them.<BR>";
    echo "More information is available at ";
    echo "<a href=RTTTL.txt target='new'>RTTTL Specification</a><br>Search <a href=";
    echo "http://www.google.com.au/search?q=free+RTTTL+ringtones&hl=en&btnG=Google+Search&meta= target=new>";
    echo "Google</a> or visit <a href=http://www.free-nokia-ring-tones-4u.com/free-nokia-ring-tones-A.html";
    echo " target=new>http://www.free-nokia-ring-tones-4u.com/</a><br></h4>";
    echo "Make sure the recipients phone supports this feature.  Polyphonic ring tones cannot be used here.";
...
?>
	    	...
	    	<br><br><b>Step 3.</b>Enter the RTTTL (Ring Tone Text Transfer Language) up to 320 characters<br>
	    	<textarea rows=5 cols=60 name=RingTone onKeyUp='textCounter(this.form.RingTone,this.form.remLen,320);'><?php
	    echo $RingTone;
	    echo "</textarea>\n<br><input readonly type=text name=remLen size=3 maxlength=3 value=320> characters left";
